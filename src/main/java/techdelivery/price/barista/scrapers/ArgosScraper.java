...
public class ArgosScraper implements PriceScraper {
    @Override
    public BigDecimal findPrice(String url) {
        try {
            Document doc = Jsoup.connect(url).get();

            Element div = doc.selectFirst("li.mainprice").child(0);
            TextNode moneyNode = (TextNode) div.childNodes().get(0);
    ...
    public boolean handlesHost(String host) {
        return host.equalsIgnoreCase("argos.ie");
    }
}
